const rawInput = `1-7 q: qqqqxvqrkbqqztlqlzq
1-3 q: cqbm
...
const output = input.filter((item) => {
  const operands = item.split(" ");
  const positions = operands[0].split("-").map((item) => parseInt(item, 10));
  const letter = operands[1].substring(0, 1);
  const password = Array.from(operands[2]);

  // const instances = [
  //   password[positions[0]-1],
  //   password[positions[1]-1]
